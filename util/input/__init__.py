...
def listen_for_escape_key():
    """Used to automatically listen for any presses of the `ESC` key in a
    background thread.
...
def listen_for_space_bar():
    """Used to automatically listen for any presses of the `SPACE` key in a
    background thread.
...
def has_esc_been_pressed():
    """Used to see if `ESC` key has been pressed.
...
def has_space_been_pressed():
    """Used to see if `SPACE` key has been pressed.
