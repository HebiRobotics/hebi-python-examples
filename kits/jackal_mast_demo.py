...
def setup_mobile_io(m: 'MobileIO'):
    ...
    # 5 (Toggle): Arm Enable
    # 6 (Momentary): Arm Z Up
    # 7 (Toggle): Gripper Control
    # 8 (Momentary): Arm Z Down

    # Axis Assignments
    # 1: Camera Mast Pan
    # 2: Camera Mast Tilt
    # 3: Gripper Rotation (In Arm Mode)
    # 4: Camera Mast Zoom
    # 5: Camera Light Level
    # 6:
    # 7: Jackal Fwd/Back
    # 8: Jackal Left/Right
    ...
    m.set_axis_label(5, '\U0001F4A1', blocking=False)
    m.set_axis_value(5, -0.9)
...
def parse_mobile_feedback(m: 'MobileIO'):
    ...
    light_level = (m.get_axis_state(5) + 1.0) / 2.0
...
if __name__ == "__main__":
    ...
    # shoulder limits [-2.2, inf]
    joint_limits[1, 0] = -2.2
    ...
    def update_mobile_ui(controller: JackalControl, new_state: JackalControlState):
        if controller.state == JackalControlState.DISCONNECTED and new_state == JackalControlState.TELEOP:
            setup_mobile_io(m)
    ...
    while base_control.running and arm_control.running and mast_control.running:
        ...
        if m.get_button_state(2):
            camera.flood_light = (m.get_axis_state(5) + 1.0) / 2.0
        else:
