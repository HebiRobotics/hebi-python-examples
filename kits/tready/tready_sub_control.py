    ...
    def parse_mobile_io_feedback(m: 'MobileIO'):
        ...
        if m.update(0.0):
            ...
            if m.get_button_state(reset_pose_btn):
                return False, TreadyInputs(home=True, torque_mode=m.get_button_state(torque_btn), torque_toggle=abs(m.get_button_diff(torque_btn)) == 1), arm_inputs, mast_inputs
            ...
            if m.get_button_state(recenter_btn):
                tready_inputs = TreadyInputs(align_flippers=True, torque_mode=m.get_button_state(
                    torque_btn), torque_toggle=abs(m.get_button_diff(torque_btn)) == 1)
            else:
                ...
                tready_inputs = TreadyInputs(
                    ...
                    torque_toggle=abs(m.get_button_diff(torque_btn)) == 1
                )
...
if __name__ == "__main__":
    ...
    base_group = lookup.get_group_from_names(
        base_family, wheel_names + flipper_names)
    ...
    load_gains(base_group, os.path.join(
        root_dir, 'gains', 'smart-tready-gains.xml'))
    ...
    if leader_arm is None or follower_arm is None:
        leader_follower_control = None
    else:
        # If the config file has a home pose use that, otherwise default
        home_pose = [1.57, -1.3, 0.0, 0.0, 1.57, 1.0, 0.0]
        if leader_cfg.user_data and 'home_pose' in leader_cfg.user_data:
            home_pose = leader_cfg.user_data['home_pose'] 
        leader_follower_control = LeaderFollowerControl(leader_arm, follower_arm, home_pose=home_pose)
    ...
    try:
        ...
        mast_group = lookup.get_group_from_names(
            mast_family, mast_module_names)
        if mast_group is None:
            raise RuntimeError(
                'Could not find mast group. Check that family and names match actuators on the network.')
        else:
    ...
    def update_mobile_io(controller: TreadyControl, new_state: TreadyControlState):
        ...
        if new_state is TreadyControlState.HOMING:
            ...
            set_mobile_io_instructions(
                m, 'Emergency Stop Activated', color="red")
    ...
    while base_control.running and \
          (leader_follower_control is None or leader_follower_control.running) and \
          (mast is None or mast_control.running):
        t = time()
        try:
            ...
            if leader_follower_control is not None:
