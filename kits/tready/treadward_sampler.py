...
class CoreSampler:
    ...
    def mast_stowed(self):
        if self.mast_cmd is not None:
            return np.mean(self.MAST_STOW_POS) - self.MAST_TOLERANCE < np.mean(self.mast_cmd.position) and np.mean(self.MAST_STOW_POS) + self.MAST_TOLERANCE > np.mean(self.mast_cmd.position)
        return np.mean(self.MAST_STOW_POS) - self.MAST_TOLERANCE < np.mean(self.mast_fbk.position) and np.mean(self.MAST_STOW_POS) + self.MAST_TOLERANCE > np.mean(self.mast_fbk.position)
    
    @property
    def mast_deployed(self):
        if self.mast_cmd is not None:
            return np.mean(self.MAST_DEPLOY_POS) - self.MAST_TOLERANCE < np.mean(self.mast_cmd.position) and np.mean(self.MAST_DEPLOY_POS) + self.MAST_TOLERANCE > np.mean(self.mast_cmd.position)
        return np.mean(self.MAST_DEPLOY_POS) - self.MAST_TOLERANCE < np.mean(self.mast_fbk.position) and np.mean(self.MAST_DEPLOY_POS) + self.MAST_TOLERANCE > np.mean(self.mast_fbk.position)

    @property
    def chain_stowed(self):
        if self.chain_cmd is not None:
            return np.mean(self.CHAIN_STOW_POS) - self.CHAIN_TOLERANCE < np.mean(self.chain_cmd.position) and np.mean(self.CHAIN_STOW_POS) + self.CHAIN_TOLERANCE > np.mean(self.chain_cmd.position)
        return np.mean(self.CHAIN_STOW_POS) - self.CHAIN_TOLERANCE < np.mean(self.chain_fbk.position) and np.mean(self.CHAIN_STOW_POS) + self.CHAIN_TOLERANCE > np.mean(self.chain_fbk.position)
    ...
    def set_chain_trajectory(self, t_now: float, ramp_time: float, p=None, v=None, trans_v=None):
        if ramp_time < 3:
            times = [t_now, t_now + ramp_time]
        ...
        self.chain_traj = hebi.trajectory.create_trajectory(times, positions, velocities, accelerations)
    ...
    def deploy_mast(self, t_now: float):
        if self.deploy_safe:
            ...
            deploy_distance = float(abs(np.mean(self.MAST_DEPLOY_POS)-np.mean(mast_pos)))
    ...
    def stow_chain(self, t_now: float):
        ...
        self.set_chain_trajectory(t_now, stow_time, p=self.CHAIN_STOW_POS, trans_v=stow_vel)
...
class CoreSamplerControl:
    ...
    def update(self, t_now: float, m_update: bool, sampler_input: 'Optional[CoreSamplerInputs]'=None):
