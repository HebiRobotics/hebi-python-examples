...
class Arm(PeripheralBody):
  ...
  def update_command(self, group_command, pose, soft_start):
    ...
    np.copyto(self._grav_comp_torque.ravel(), math_utils.get_grav_comp_efforts(self._robot, positions, -pose[2, 0:3]))
  ...
  def reset_state(self):
    ...
    np.copyto(self._grip_pos, self._new_grip_pos)
    np.copyto(self._joint_angles, home_angles.reshape((4, 1)))
