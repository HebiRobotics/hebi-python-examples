...
class Igor2Config(object):
    ...
    To select a different joystick selection strategy, you must call one
    of the select_joystick_by_* methods.
    """
    ...
    def select_joystick_by_index(self, index):
        """Set the joystick selection strategy to select a joystick by
        index."""
        self.__find_joystick_strategy = _create_controller_selector('index', index)
        self.__controller_mapping = _default_joystick_mapping

    def select_first_available_joystick(self):
        """Set the joystick selection strategy to select the first available
        joystick."""
